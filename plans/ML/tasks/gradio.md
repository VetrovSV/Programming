...
3. Показывать одну диаграмму рассеяния: точки данных из обучающей выборки + введённая пользователем точка (выделена). Ось X — наиболее важный признак, ось Y — целевая переменная.
4. Отображать:
...
#### Рекомендации
* **Формат модели:**  `joblib.dump(model, "model.joblib")` / `joblib.load(...)`. Указывайте версию sklearn в `requirements.txt`.
* Загружайте модель в оперативную память заранее, при старте сервера.
* **LaTeX в Gradio:** можно выводить Markdown с формулой LaTeX (например `r"$y = 2.34x + 0.56$"`)
* Если вы ещё не выполнили задание по линейной регрессии, то временно используйте произвольную линейную функцию.
* Примеры gradio: [examples/gradio](../../../examples/gradio/)
* Для изображения прямой используйте функцию `plot` из matplotlib. Её можно вызывать из объекта типа `matplotlib.axes.Axes`
