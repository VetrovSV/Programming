...
- использование только `assert`. 
  - Простые проверки которые можно быстро написать
  - Нужно вручную организовывать запуск, 
  - Проверяются по одному assert
  - Нет отчётов о тестировании
  - Нет структуры тестов
  - Неудобно запускать большие наборы тестов
- `unittest` — пакет из стандартной библиотеки
  - тесты строятся на основе специального класса
  - есть отчёты о запуске тестов
  - много встроенных assert-методов
  - хорошая интеграция с IDE и CI
  - нужно писать шаблонный код
- `pytest` — один из самых популярных фреимворков
  - тесты запускаются отдельной командой
  - автоматическое обнаружение тестовых модулей и функций
  - есть отчёты о запуске тестов
  - тесты строятся на основе простых assert
  - есть фикстуры (заранее подготовленные типовые данные)
  - разнообразные плагины


**Фикстура** (fixture — приспособление) в тестировании — это код, который готовит окружение для теста: данные, объекты, соединения, файлы, состояние базы и т.п., которые нужны тесту перед выполнением.

Фикстура нужна там, где приходится создавать в нескольких тестах одинаковые тестовые данные, открывать соединения, файлы и т.п. Фикстура автоматически вызывается при запуске тестовых функций.

См. пример фикстуры ниже.
...
**Простой пример** — [10_example_pytest](./10_example_pytest)
...
def test_calc_hypotenuse_parametrized(a, b, expected):
    ...
    # функция test_calc_hypotenuse_parametrized будет запущена для каждого варианта тестовых данных, т.е. 3 раза
